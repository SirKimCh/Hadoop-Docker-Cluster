...
echo 'ubuntu ALL=(ALL) NOPASSWD: SETENV: /opt/hadoop/bin/*, /opt/hadoop/sbin/*, /usr/bin/jps' | sudo tee /etc/sudoers.d/hadoop
...
**Lỗi `sudo: sorry, you are not allowed to preserve the environment`:**
Do dùng `sudo -E` mà sudoers không có tag `SETENV:`. Đã sửa trong script — không dùng `-E` nữa. Nếu cấu hình sudoers thủ công, cần thêm `SETENV:`:
```
ubuntu ALL=(ALL) NOPASSWD: SETENV: /opt/hadoop/bin/*, /opt/hadoop/sbin/*, /usr/bin/jps
```
Chạy lại `./scripts/setup-worker-sudo.sh` để cập nhật.

**Lỗi `secondarynamenode is running as process XXXX`:**
Script tự dừng daemon cũ trước khi khởi động. Nếu vẫn lỗi, dừng thủ công:
```bash
sudo /opt/hadoop/bin/hdfs --daemon stop secondarynamenode
sudo /opt/hadoop/bin/hdfs --daemon stop namenode
sudo /opt/hadoop/bin/yarn --daemon stop resourcemanager
```
