...
START_TIME=$(date +%s)
hadoop jar retail-q1.jar OnlineRetailQ1 /data/input/retail/online_retail_II.csv /data/output/retail-q1
END_TIME=$(date +%s)
ELAPSED=$((END_TIME - START_TIME))

mkdir -p /result
echo "$NODES,$ELAPSED" >> /result/q1_execution_times.csv

RES_DIR="/result/$(date +%d-%m-%Y_%H-%M)_Q1_${1}Nodes"
mkdir -p $RES_DIR

hdfs dfs -get /data/output/retail-q1/part-r-00000 $RES_DIR/
