...
LOG_FILE="/result/q2_execution_times.csv"
...
for MAPPERS in $MAPPER_COUNTS; do
    ...
    for RUN in $(seq 1 $RUNS); do
        ...
        START_TIME=$(date +%s)
        hadoop jar retail-q2.jar OnlineRetailQ2 /data/input/retail/online_retail_II.csv /data/output/retail-q2 $LINES_PER_MAP
        END_TIME=$(date +%s)
...
RES_DIR="/result/${TIMESTAMP}_Q2_Experiment"
mkdir -p $RES_DIR
hdfs dfs -get -f /data/output/retail-q2/part-r-00000 $RES_DIR/
cp $LOG_FILE $RES_DIR/
