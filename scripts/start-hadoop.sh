...
if [ "$(id -u)" -ne 0 ]; then
    ...
    exec sudo JAVA_HOME="$JAVA_HOME" HADOOP_HOME="$HADOOP_HOME" "$0" "$@"
fi
...
for W in "${WORKERS[@]}"; do
    ssh "$W" "export JAVA_HOME=$JAVA_HOME && $HADOOP_HOME/bin/hdfs --daemon start datanode"
...
for W in "${WORKERS[@]}"; do
    ssh "$W" "export JAVA_HOME=$JAVA_HOME && $HADOOP_HOME/bin/yarn --daemon start nodemanager"
